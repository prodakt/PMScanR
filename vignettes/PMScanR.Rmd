...
This function is an automatic form of analysis loading, which searches the sequence for protein motifs present on it, but there is also an alternative option where the user can manually point the path to each of the flies and the version of the operation system.

```{r run-ps-scan-manual, eval=FALSE}
# Manual paths - definitely set eval=FALSE as these paths are examples
# Path to the PS-Scan perl script (e.g., 'ps_scan/ps_scan.pl')
# ps_scan <- "ps_scan/ps_scan.pl"
# Path to the PROSITE database file (e.g., 'ps_scan/prosite.dat')
# patterns_dat <- "ps_scan/prosite.dat"
# Default output format for PS-Scan (PSA - PROSITE scan ASCII)
# out_format <- "psa"
# Path to the PFSCAN executable (Windows, e.g., 'ps_scan/pfscan.exe') - for # Windows version
# pf_scan <- "ps_scan/pfscan.exe"

# runPsScan(in_file = fasta_file, out_format = 'gff', out_file = "results_pfscan.gff",
#           ps_scan = ps_scan, patterns_dat = patterns_dat, pf_scan = pf_scan , OS = "WIN")
```

After analysis, which finally searched for protein motifs located on the input file sequence, the two functions below can be used to convert the files into GFF format:

#### `readPM.prosite()`

A function that enables an input file in prosite format to be converted into a GFF format file, which is a more universal format used for analysis and visualisation such as heatmap or pie chart. *(Note: Example code not provided for this specific function in the text)*

#### `readPM.psa()`

A function that enables an input file in psa format *(Note: text says 'prosite format' here, but function name implies PSA)* to be converted into a GFF format file, which is a more universal format used for analysis and visualisation such as heatmap or pie chart. You can point the path to the output file generated by the actual analysis:
...
if (file.exists(motifs_psa_file)) {
  psaGFF <- readPM.psa(motifs_psa_file)
...
Function used for another type of data visualisation which is a seqlogo generated based on the frequency of occurrence of individual amino acid residues in a selected part of the sequence, that is the defined region in selected sequences. Function used for a psa format input file, after using the `prepare_segments()` function *(Note: `prepare_segments` not described elsewhere)*. This function extracts the protein motifs present on the sequence thus preparing the file for seqlogo creation. *(Note: Example seems to use FASTA input directly, not PSA)*
...
if (exists("fasta_file") && file.exists(fasta_file)) {
  ...
  if (length(seq) > 0) {
     seqShort <- extract_segments(seq = seq, from_pos = from_pos, to_pos = to_pos)
...
Function used for another type of data visualisation which is a seqlogo generated based on the frequency of occurrence of individual amino acid residues in a selected part of the sequence, that is the defined region in selected sequences. The function used for the input file in FASTA format *(Note: text says FASTA, but example uses PSA output)*, which extracts the protein motifs present on the sequence in the file, thus preparing them for data visualisation using seqlogo. Example of usage for creating sequence logo for a selected region from FASTA: *(Note: Example title says 'selected region from FASTA', but code uses `motifs_psa`)*
