...
detectOs <- function() {
  ...
  if (grepl("windows", sys_info)) return("WIN")
  if (grepl("linux", sys_info)) return("LINUX")
  if (grepl("darwin", sys_info)) return("MAC")
  ...
  if (osType == "Darwin") {
    ...
    if (file.exists(wmicPath)) {
      driveCaptions <- tryCatch(
        system2(wmicPath, args = "logicaldisk get Caption", stdout = TRUE, stderr = FALSE),
        error = function(e) character(0)
      )
      volumeNames <- tryCatch(
        system2(wmicPath, args = "logicaldisk get VolumeName", stdout = TRUE, stderr = FALSE),
        error = function(e) character(0)
      )

      driveCaptions <- driveCaptions[driveCaptions != "" & !grepl("Caption", driveCaptions)]
      volumeNames <- volumeNames[volumeNames != "" & !grepl("VolumeName", volumeNames)]

      if (length(driveCaptions) > 0 && length(driveCaptions) == length(volumeNames)) {
    ...
    ps_command <- "[System.IO.DriveInfo]::GetDrives() | ForEach-Object { $_.Name + ',' + $_.VolumeLabel }"
    driveInfo <- tryCatch(
      system2("powershell", args = c("-Command", ps_command), stdout = TRUE, stderr = FALSE),
      error = function(e) character(0)
    )
    ...
    if (length(driveInfo) > 0) {
      ...
      driveNames <- paste0(drive_df[, 2], " (", gsub("/$", "", logicalDrives), ")")
      names(logicalDrives) <- driveNames
    } else {
