...
matrix2hm <- function(input, x = NULL, y = NULL) {
    ...
    mat_sel <-
        as.data.frame(input) %>% mutate(across(.cols = everything(), .fns = ~ -1))
    mat_sel <- as.matrix(mat_sel)
    mat_sel[rownames(mat_sel) %in% y, ] <-
    ...
    hp <- plot_ly(
        ...
        add_segments(
            x = colnames(mat_sel),
            xend = colnames(mat_sel),
            y = rownames(mat_sel)[1],
            yend = rownames(mat_sel)[nrow(mat_sel)],
            line = list(color = "black", width = 0.3),
            inherit = FALSE
        ) %>%
        add_segments(
            y = rownames(mat_sel),
            yend = rownames(mat_sel),
            x = colnames(mat_sel)[1],
            xend = colnames(mat_sel)[ncol(mat_sel)],
            line = list(color = "black", width = 0.3),
            inherit = FALSE
        ) %>%
        layout(
