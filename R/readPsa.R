...
readPsa <- function(psa_file) {
    ...
    if (!is.na(first_tab_line)) {
        psa_lines <- psa_lines_all[seq_len(first_tab_line - 1)]
    } else {
    ...
    for (i in seq_along(header_indices)) {
        ...
        header_line <- block[1]
        sequence_lines <- if (length(block) > 1)
            block[-1]
        else
            ""
        ...
        if (is.na(matches[1, 1])) {
            warning(sprintf("Could not parse header line: %s", header_line),
                    call. = FALSE)
            next
        }
        ...
        if (is.na(start_pos) || is.na(end_pos)) {
            warning(
                sprintf(
                    "Could not convert start/end to integer in header: %s",
                    header_line
                ),
                call. = FALSE
            )
