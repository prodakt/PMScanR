...
test_that("freqPie calculates data correctly for plotting", {
  ...
  sample_data <- data.frame(
    Name = c("ZINC_FINGER", "EGF_DOMAIN", "ZINC_FINGER", "ZINC_FINGER", "KINASE")
  )
  ...
  expected_data$Name_and_procent <- paste0(
    expected_data$Name, " (", round(expected_data$percentage, 1), "%)"
  )
  expected_data <- expected_data[order(expected_data$Name, decreasing = TRUE), ]
